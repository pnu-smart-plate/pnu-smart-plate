...
function selectFood(food) {
  ...
  document.getElementById('food-calories').textContent = calculatePerServing(
      food.calories) + ' kcal';
  document.getElementById(
      'food-nutrients').textContent = `단백질 ${calculatePerServing(
      food.protein)}g, 지방 ${calculatePerServing(
      food.fat)}g, 탄수화물 ${calculatePerServing(food.carbs)}g`;
}

function updateServingSize() {
  const selectedFood = foodData.find(
      food => food.name === document.getElementById(
          'analyzed-food').textContent);
  if (selectedFood) {
